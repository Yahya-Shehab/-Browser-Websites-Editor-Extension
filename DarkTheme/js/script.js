...
window.addEventListener("DOMContentLoaded", () => {
  ...
  function sendMessageToBackground() {
    chrome.tabs.query({ active: true, currentWindow: true }, (tabs) => {
      chrome.runtime.sendMessage({
        message: "darkThemeToggle",
        name: toggleButton.dataset.toggle,
        id: tabs[0].id,
        url: tabs[0].url,
      });
    });
...
function setTimer() {
  ...
  if (!validateFrom || !validateTo) {
    ...
    let fromTime = new Date(
      ...
      } ${currentTime.getDate()}, ${currentTime.getFullYear()} ${fromHour}:${fromMinute}:00`
    );
    ...
    let toTime = new Date(
      `${
        monthNames[currentTime.getMonth()]
      } ${currentTime.getDate()}, ${currentTime.getFullYear()} ${toHour}:${toMinute}:00`
    );
...
addtionButton.addEventListener("click", () => {
  ...
  chrome.tabs.query({ active: true, currentWindow: true }, (tab) => {
    ...
    chrome.storage.sync
      .get("whiteListWebsite")
      .then((data) => {
        if (data["whiteListWebsite"]) {
          let arr = JSON.parse(data["whiteListWebsite"]);
          for (let i = 0; i < arr.length; i++) {
            if (urlParse.hostname == arr[i].hostName) return;
          }
        }
        ...
        chrome.storage.sync.set({
          whiteListWebsite: JSON.stringify(whiteListWebsites),
        });
      })
      .then(() => {
        addToWhiteList();
